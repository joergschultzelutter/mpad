...
def testcall(message_text: str, from_callsign: str):
    success, aprsdotfi_api_key, openweathermapdotorg_api_key, aprsis_callsign, aprsis_passcode, dapnet_login_callsign, dapnet_login_passcode = read_program_config()
    assert success
    ...
    if success:
        ...
        response_parameters.update({"aprsdotfi_api_key": aprsdotfi_api_key, "openweathermapdotorg_api_key": openweathermapdotorg_api_key, "dapnet_login_callsign": dapnet_login_callsign, "dapnet_login_passcode": dapnet_login_passcode})
        logger.info("Response:")
        logger.info(
            pformat(generate_output_message(
                response_parameters
            )
            )
        )
