...
def download_repeatermap_raw_data_and_write_it_to_disc(url: str = "http://www.repeatermap.de/api.php",
    ...
    if resp.status_code == 200:
        try:
            ...
            log_to_stderr(f"Cannot write repeatermap.de data to local disc file '{repeatermap_raw_data_file}'")
    return success


def read_repeatermap_raw_data_from_disk(repeatermap_raw_data_file: str ="repeatermap.raw"):
    """
...
def calculate_band_name(frequency: float):
    ...
    for band in band_dictionary:
        if frequency >= band_dictionary[band]['from'] and frequency <= band_dictionary[band]['to']:
...
def create_enriched_mpad_repeatermap_data(repeatermap_raw_json_content: str):
    ...
    for raw_entry in raw_repeatermap_dictionary['relais']:
        mode = rx_frequency = tx_frequency = elevation = latitude = \
            longitude = remarks = qth = id = locator = \
            callsign = band_name = None
        ...
        if id and not 'mmdvm' in remarks.lower():
            mpad_repeater_dict[f"{id}"] = {'locator': locator,
                                            'latitude': latitude,
                                            'longitude': longitude,
                                            'mode': mode,
...
def write_mpad_repeatermap_data_to_disc(mpad_repeatermap_json: str,
    ...
    try:
        ...
        log_to_stderr(f"Cannot write native repeatermap data to local disc file '{mpad_repeatermap_filename}'")
    return success

def read_mpad_repeatermap_data_from_disc(mpad_repeatermap_filename: str = "repeatermap.mpad"):
    """
...
def get_nearest_repeater(latitude: float, longitude: float,
...
if __name__ == '__main__':
    ...
    if success:
        success, local_repeatermap_json = create_enriched_mpad_repeatermap_data(repeatermap_dot_de_content)
        if success:
            success = write_mpad_repeatermap_data_to_disc(local_repeatermap_json)

    success, mpad_repeatermap_content = read_mpad_repeatermap_data_from_disc()
    if success:
        mpad_repeater_dictionary = json.loads(mpad_repeatermap_content)

        id = get_nearest_repeater(latitude=51.8458575,
                                    longitude=8.2997425,
        ...
        if id:
            ...
            distance, bearing, direction = Haversine(51.8458575,8.2997425,latitude,longitude,'metric')
            print (f"Nearest repeater: {qth} {round(latitude,2)}/{round(longitude,2)}, "
